...
public class SportcheckSignOutWihoutPageObjectModelTest {
	...
	public void sportcheckLoginToastMessageTest() {
		...
		driver.findElement(By.cssSelector(".gigya-composite-control-loginID input")).sendKeys(loginEmail);
		driver.findElement(By.cssSelector(".gigya-composite-control-password input")).sendKeys(loginPassword);
		driver.findElement(By.cssSelector(".gigya-composite-control-submit input")).click();
		
		// At this moment we need to wait until the spinner screen is gone, because it would intercept
		// the clicks
		new WebDriverWait(driver, Duration.ofSeconds(10))
		.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector(".nl-spinner__content")));
		...
		Assert.assertTrue(driver.findElement(By.cssSelector(".nl-sm-block .nl-pencil-banner__login")).isDisplayed());
	}
